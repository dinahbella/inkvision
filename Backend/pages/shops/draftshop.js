...
export default function DraftShop() {
  ...
  const currentShops = filteredBlogs.slice(indexOfFirstBlog, indexOfLastBlog);

  // Filter published blogs
  const publishedBlogs = currentShops.filter((ab) => {
    console.log("Product Status:", ab.status); // Debugging
    return ab.status === "Draft";
  });
    ...
    <div className="blogspage">
      <div className="titledashboard flex flex-sb">
        ...
        <div className="breadcrumb">
          <SiBloglovin /> <span>/</span> <span>Draft Product</span>
        </div>
      </div>
      <div className="blogstable">
        <div className="flex gap-2 mb-1">
          <h2 className="flex">Search Products:</h2>
          <input
            type="text"
        ...
        {publishedBlogs.length === 0 ? null : (
          <div className="blogpagination">
            ...
            <button
              onClick={() => paginate(currentPage + 1)}
              disabled={currentShops.length < perPage}
            >
              Next
            </button>
          </div>
