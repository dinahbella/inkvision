...
export default function Shops() {
  ...
  const currentShop = filteredProjects.slice(indexOfFirstBlog, indexOfLastBlog);

  // Filter published blogs
  const publishedBlogs = currentShop.filter((ab) => {
      ...
      <div className="blogspage">
        ...
        <div className="blogstable">
          <div className="flex gap-2 mb-1">
            <h2 className="flex">Search Shop:</h2>
            <input
              type="text"
          ...
          {publishedBlogs.length === 0 ? null : (
            <div className="blogpagination">
              ...
              <button
                onClick={() => paginate(currentPage + 1)}
                disabled={currentShop.length < perPage}
              >
                Next
              </button>
            </div>
